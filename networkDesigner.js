...
d3.json("network.json", function(network) {
    ...
    function animateBranch(nodeData) {
        var links = svg.selectAll(".gradient-link")
            .filter(function(d) {
                return nodeData.sourceLinks.indexOf(d) >
                    -1
            });
...
function dragmove(d) {
    ...
    if (d3.event.sourceEvent.button == 0) { // If left mouse button
        d3.select(this).attr("transform",
            "translate(" + (
                d.x = Math.max(0, Math.min(width - d.dx, d3.event
                    .x))
            ) + "," + (
                d.y = Math.max(0, Math.min(height - d.dy, d3.event
                    .y))
            ) + ")");
        ...
        svg.selectAll(".gradient-link").filter(function(l) {
            return l.target == d;
        }).attr("d", path);
