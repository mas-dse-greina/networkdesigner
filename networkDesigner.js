        ...
        links
            ...
            .each("end", function() {
                nextLayerNodeData.forEach(function(d) {
                    animateBranch(d); // Recurse to animate the successive branches
                });
            });
    }var margin = {
        top: 0,
...
d3.json("network.json", function(network) {
            ...
            function setDash(d) {
                function animateBranch(nodeData) {

                    var gradientLink = svg.append("g").selectAll(
                            ".gradient-link")
                        .data(network.links)
                        .enter().append("path")
                        ...
                        .style('stroke', function(d) {
                            var sourceColor = color(d.source
                                .name.replace(/ .*/,
                                    "")).replace("#",
                                "");
                            var targetColor = color(d.target
                                .name.replace(/ .*/,
                                    "")).replace("#",
                                "");
                            var id = 'c-' + sourceColor +
                                '-to-' + targetColor;
                    ...
                    var link = svg.append("g").selectAll(
                            ".link")
                        .data(network.links)
                        .enter().append("path")
                        .attr("class", "link")
                    ...
                    var node = svg.append("g").selectAll(
                            ".node")
                        .data(network.nodes)
                        .enter().append("g")
                        .attr("class", "node")
                    ...
                    node.append("rect")
                        ...
                        .append("title")
                        .text(function(d) {
                            return "# inputs = " + (d.targetLinks
                                    .length) +
                                "\n# outputs = " + (d.sourceLinks
                                    .length);
                        })
            ...
            function dragmove(d) {
                ...
                if (d3.event.sourceEvent.button == 0) { // If left mouse button
                    d3.select(this).attr("transform",
                        "translate(" + (
                            d.x = Math.max(0, Math.min(width -
                                d.dx, d3.event.x))
                        ) + "," + (
                            d.y = Math.max(0, Math.min(height -
                                d.dy, d3.event.y))
                        ) + ")");

                    // Now update the links with the new node path
                    ...
                    svg.selectAll(".gradient-link").filter(
                        function(l) {
                            return l.target == d;
                        }).attr("d", path);
